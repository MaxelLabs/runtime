# 代码示例总览

## 概述

本目录包含了Maxellabs Runtime v2的完整代码示例，展示了RHI、Math、Specification三个库的协作使用。每个示例都是完整可运行的，包含详细的注释和最佳实践说明。

## 🎨 完整渲染场景

**文件**: `complete-rendering-scene.md`

**功能**: 展示如何使用RHI创建WebGL设备和资源，使用Math库进行3D变换和矩阵运算，实现完整的渲染管线设置

**关键特性**:
- RHI设备初始化和资源管理
- Math对象池优化
- 相机控制和视图变换
- 多模式渲染（光照、线框、深度可视化）
- 性能监控和错误处理
- 实时用户交互控制

**技术要点**:
- WebGPU设备创建和配置
- 顶点缓冲区和索引缓冲区管理
- 渲染管线和绑定组
- 变换矩阵计算
- 视口和渲染通道管理

## 🎬 动画系统

**文件**: `animation-system.md`

**功能**: 完整的关键帧动画系统，包括多层动画混合、缓动函数和性能优化

**关键特性**:
- Specification驱动的动画数据结构
- 多种缓动函数（线性、二次、三次、弹性、反弹等）
- 关键帧插值器（标量、Vector3、Quaternion）
- 多层动画混合（覆盖、加性）
- 动画状态机系统
- 性能优化和对象池使用

**技术要点**:
- SLERP四元数插值
- 球面线性插值实现
- 时间管理和动画循环
- 权重混合系统
- 内存优化策略

## 📦 资产管理

**文件**: `asset-pipeline.md`

**功能**: 完整的资产加载和管理流程，包括USD资产解析、RHI资源创建、内存优化和异步加载

**关键特性**:
- 多格式支持（USD、GLTF、OBJ、FBX等）
- 异步加载和流式加载
- 内存管理和GPU资源优化
- USD资产完整解析器
- 跨平台兼容性处理
- 错误处理和降级方案

**技术要点**:
- USD格式解析（ASCII和二进制）
- 资产依赖管理
- GPU资源创建和绑定
- 内存预算管理
- 预加载和缓存策略

## ⚡ 性能优化

### 性能优化集合

**文件**: `performance-optimization.md` (主文档)
**拆分文档**:
- [性能分析器](./performance-analyzer.md) - 实时FPS、GPU、CPU和内存监控系统
- [Math对象池优化](./math-object-pool-optimization.md) - 高性能对象池管理，减少GC压力
- [RHI命令优化器](./rhi-command-optimizer.md) - GPU命令批处理和状态缓存优化
- [SIMD优化器](./simd-optimizer.md) - 利用SIMD指令加速数学运算
- [内存泄漏检测器](./memory-leak-detector.md) - 内存使用监控和泄漏检测
- [性能优化完整演示](./performance-optimization-demo.md) - 集成所有优化技术的综合示例

**功能**: 全面的性能优化技术，包括RHI命令优化、Math对象池深度使用、SIMD优化、渲染性能分析和内存泄漏检测

**关键特性**:
- 实时性能监控和分析
- RHI命令批处理和状态优化
- Math对象池智能管理
- SIMD向量化计算优化
- 内存泄漏自动检测
- 性能基准测试和对比

**技术要点**:
- GPU命令队列优化
- 对象池动态调整
- SIMD指令自动检测和使用
- 内存使用趋势分析
- 循环引用检测

**优化效果**:
- 数学运算性能提升: 300%
- 内存分配减少: 62%
- GPU状态切换减少: 80%
- 帧率提升: 33%

## 🚀 快速开始

### 环境要求

- **TypeScript**: >= 4.5
- **WebGPU**: 支持WebGPU的浏览器（Chrome 113+、Edge 113+）
- **Node.js**: >= 16.0（用于开发环境）

### 运行示例

1. **克隆项目**
```bash
git clone https://github.com/maxellabs/runtime.git
cd runtime
```

2. **安装依赖**
```bash
npm install
```

3. **启动开发服务器**
```bash
npm run dev
```

4. **在浏览器中访问示例**
- 完整渲染场景: `http://localhost:3000/examples/rendering-scene`
- 动画系统: `http://localhost:3000/examples/animation`
- 资产管理: `http://localhost:3000/examples/assets`
- 性能优化: `http://localhost:3000/examples/performance`

### 运行单个示例

每个示例都可以独立运行：

```typescript
// 完整渲染场景示例
import { CompleteRenderingScene } from './complete-rendering-scene';

const scene = new CompleteRenderingScene('canvas');
await scene.start();
```

## 📊 性能对比

### 基准测试结果

| 优化技术 | 性能提升 | 适用场景 |
|---------|---------|----------|
| SIMD向量运算 | 2-4x | 大批量数学计算 |
| 对象池 | 3-8x | 频繁对象创建/销毁 |
| 命令批处理 | 2-5x | 大量绘制调用 |
| 实例化渲染 | 10-50x | 相同几何体多次渲染 |
| 内存优化 | 20-50% | 内存敏感应用 |

### 内存使用优化

```typescript
// 优化前 - 每帧创建新对象
for (let i = 0; i < 1000; i++) {
    const matrix = new MMath.Matrix4();
    // 使用matrix...
}

// 优化后 - 使用对象池
const matrices = MathPool.getMatrix4Batch(1000);
// 使用matrices...
MathPool.releaseMatrix4Batch(matrices);
```

## 🎯 最佳实践

### 1. 资源管理

```typescript
// ✅ 正确的资源生命周期
const resource = resourceManager.getResource(assetId);
try {
    // 使用资源...
} finally {
    resourceManager.releaseResource(assetId);
}
```

### 2. 性能监控

```typescript
// ✅ 持续性能监控
analyzer.beginFrame();
// 渲染代码...
analyzer.endFrame();

if (analyzer.getMetrics().frame.fps < 30) {
    // 触发降质量策略
}
```

### 3. 错误处理

```typescript
// ✅ 健壮的错误处理
try {
    await assetLoader.loadAsset(url);
} catch (error) {
    console.error('加载失败，使用备用资产:', error);
    await assetLoader.loadAsset(fallbackUrl);
}
```

### 4. 内存管理

```typescript
// ✅ 使用对象池避免GC
const vector = MathPool.vec3(x, y, z);
// 使用vector...
MathPool.releaseVec3(vector);
```

## 🔄 从Demo迁移

如果你已经在使用现有的Demo，这里有一些迁移建议：

### 简单渲染迁移

```typescript
// 旧的Demo方式
const device = new WebGLDevice(canvas);
const pipeline = device.createRenderPipeline({...});

// 新的优化方式
const scene = new CompleteRenderingScene('canvas');
await scene.init(); // 自动处理设备初始化
```

### 数学运算迁移

```typescript
// 旧的方式
const vector = new MMath.Vector3();

// 新的优化方式
const vector = MathPool.vec3();
MathPool.releaseVec3(vector);
```

### 资产加载迁移

```typescript
// 旧的方式
const geometry = loadGeometry(url);

// 新的方式
const asset = await assetManager.loadAsset(url);
const resource = resourceManager.createMeshResource(asset);
```

## 🐛 常见问题

### Q: SIMD不支持怎么办？

A: SIMDWrapper会自动检测SIMD支持并优雅降级到标准实现。

### Q: 对象池什么时候应该使用？

A: 当对象创建频率超过每秒1000次时，建议使用对象池。

### Q: 如何调试性能问题？

A: 使用PerformanceAnalyzer获取详细的性能报告和建议。

### Q: 内存泄漏如何检测？

A: 启用GlobalMemoryLeakDetector，它会自动追踪和分析内存使用。

## 📚 相关文档

- [RHI详细文档](../rhi/)
- [Math库文档](../math/)
- [Specification文档](../specification/)
- [最佳实践指南](../../guides/best-practices.md)
- [常见问题解答](../../guides/faq.md)

## 🤝 贡献

欢迎提交Issue和Pull Request来改进这些示例。在提交代码时，请确保：

1. 代码符合项目的TypeScript规范
2. 添加了适当的注释和文档
3. 包含必要的错误处理
4. 通过了性能基准测试

## 📄 许可证

这些示例遵循项目的MIT许可证。